...
namespace dal {
    ...
    void ModelStatic::renderGeneral(const UnilocGeneral& uniloc, std::list<ActorInfo>& actors) const {
    ...
    void ModelStatic::renderDepthMap(const UnilocDepthmp& uniloc, std::list<ActorInfo>& actors) const {
        if ( !this->isReady() ) return;
...
namespace dal {
    ...
    void ModelAnimated::BoneTransform(float TimeInSeconds) {
        ...
        const auto result = this->m_animations[0].sample(AnimationTime, this->m_jointInterface, this->m_globalInvMat);

        for ( size_t i = 0; i < result.size(); i++ ) {
            this->m_jointInterface.setFinalTransform(i, result[i]);
        }
